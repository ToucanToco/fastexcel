...
use calamine::{open_workbook_auto, open_workbook_auto_from_rs, Data, DataRef, Range, Reader, Sheets, Table};
...
impl ExcelSheets {
	...
	fn table_names(&mut self, sheet_name: Option<&str>) -> FastExcelResult<Vec<String>> {
		match self {
			Self::File(sheets) => {
				extract_table_names(sheets, sheet_name)?
	...
	fn get_table(&mut self, name: &str) -> FastExcelResult<Table<Data>> {
		match self {
			Self::File(sheets) => {
				extract_table_range(name, sheets)?
			}
			Self::Bytes(sheets) => {
				extract_table_range(name, sheets)?
			}
